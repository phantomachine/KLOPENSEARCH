...
% % 
% % GENERALIZED NASH BARGAINING CASE
% %
% % INPUT: none
% %
...
% Define parameters
syms THETA_B C ...
    ALPHA BETA GAMA ETA DELTA EPSILON THETA PHI SIGMA KAPPA ...
    TAU_X TAU_H TAU_K ...
    A B b ...
    ZBAR SG ...
    RHO_Z1   RHO_Z12...
...
% g:
g_NASH1 = g_nash(q1,K1,Z1,THETA_B,PHI,ETA,b,C);
g_NASH2 = g_nash(q2,K2,Z2,THETA_B,PHI,ETA,b,C);

% g_qz:
g_PROP_qz1 = g_prop_q(qz1,K1,Z1,THETA_B,PHI,ETA,b,C);
g_PROP_qz2 = g_prop_q(qz2,K2,Z2,THETA_B,PHI,ETA,b,C);

% g_q:
g_NASH_q1 = g_nash_q(q1,K1,Z1,THETA_B,PHI,ETA,b,C);
g_NASH_q2 = g_nash_q(q2,K2,Z2,THETA_B,PHI,ETA,b,C);

% g_q+:
g_NASH_q1p = g_nash_q(q1p,K1p,Z1p,THETA_B,PHI,ETA,b,C);
g_NASH_q2p = g_nash_q(q2p,K2p,Z2p,THETA_B,PHI,ETA,b,C);

% Gamma function in DM
gamma_NASH_K1p = gamma_nash(q1p,K1p,Z1p,THETA_B,PHI,ETA,b,C);
gamma_PROP_Kz1p = gamma_nash(qz1p,K1p,Z1p,THETA_B,PHI,ETA,b,C);

gamma_NASH_K2p = gamma_nash(q2p,K2p,Z2p,THETA_B,PHI,ETA,b,C);
gamma_PROP_Kz2p = gamma_nash(qz2p,K2p,Z2p,THETA_B,PHI,ETA,b,C);
...
f1 = MU_X1 - BETA*( MU_X1p*( 1 + (1-TAU_K)*(r1p - DELTA) ) ...
                        - SIGMA*(1+TAU_X)*(KAPPA*gamma_NASH_K1p ...
                                + (1-KAPPA)*(1-THETA_B)*gamma_PROP_Kz1p) );
f2 = MU_X2 - BETA*( MU_X2p*( 1 + (1-TAU_K)*(r2p - DELTA) ) ...
                        - SIGMA*(1+TAU_X)*(KAPPA*gamma_NASH_K2p ...
                                + (1-KAPPA)*(1-THETA_B)*gamma_PROP_Kz2p) );
% Pins down (K1+, K2+)
...
f3 = MU_X1 ...
    ...
    *( 1 - SIGMA*KAPPA + SIGMA*KAPPA*MU_q1p/g_NASH_q1p );
f4 = MU_X2 ...
    - BETA*MU_X2p*(P2/P2p)*(1/psi2)...
    *( 1 - SIGMA*KAPPA + SIGMA*KAPPA*MU_q2p/g_NASH_q2p );
% Pins down (q1, q2)
% KAPPA: Added 04/05/2010
...
Ptilde_q1 = ((A*psi1/(P1*(1-TAU_H)*w1))^(-1))*g_NASH_q1 / q1;

% Home credit trades: price
Ptilde_qz1 = ((A*psi1/(P1*(1-TAU_H)*w1))^(-1))*g_PROP_qz1 / qz1;

% Foreign money trades: price
Ptilde_q2 = ((A*psi2/(P2*(1-TAU_H)*w2))^(-1))*g_NASH_q2 / q2;

% Foreign credit trades: price
Ptilde_qz2 = ((A*psi2/(P2*(1-TAU_H)*w2))^(-1))*g_PROP_qz2 / qz2;


% Define the share of DM output
NTS_s1 = 1-Pyh1*FF1/(Pyh1*FF1+SIGMA*(KAPPA*Ptilde_q1*q1...
                                            +(1-KAPPA)*Ptilde_qz1*qz1));
NTS_s2 = 1-Pyf2*FF2/(Pyh2*FF2+SIGMA*(KAPPA*Ptilde_q2*q2...
                                            +(1-KAPPA)*Ptilde_qz2*qz2));
...
f17 = A*psi1/(P1*(1-TAU_H)*w1) - g_NASH1; 
f18 = A*psi2/(P2*(1-TAU_H)*w2) - g_NASH2;
% Pins down (P1, P2)
...
f42 = Agg_Y1 - (Pyh1*FF1+SIGMA ...
                *(KAPPA*Ptilde_q1*q1+(1-KAPPA)*Ptilde_qz1*qz1))/YPI1;
f43 = Agg_Y2 - (Pyf2*FF2+SIGMA...
                *(KAPPA*Ptilde_q2*q2+(1-KAPPA)*Ptilde_qz2*qz2))/YPI2;
