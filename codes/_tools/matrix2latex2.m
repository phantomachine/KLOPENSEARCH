function matrix2latex2(matrix, filename, rowmath, colmath, varargin)

% Modified: T. Kam, Feb 10 2006; May 2009- added rowmath, colmath options
...
% Enjoy life!!!
    ...
    if(~isempty(colLabels))
        ...
        for w=1:width-1
            if colmath == 0
                fprintf(fid, '%s&', colLabels{w});     % Text mode
            else
                fprintf(fid, '$%s$ &', colLabels{w});  % Math mode
            end
        end
        if colmath == 0
            fprintf(fid, '%s\\\\\\hline\r\n', colLabels{width});
    ...
    for h=1:height
        if(~isempty(rowLabels))
           ...
           if rowmath == 0
                fprintf(fid, '%s&', rowLabels{h});  % TK: row labels are  text
           else
                fprintf(fid, '$%s$ &', rowLabels{h});  % TK: row labels are  text
           end
        end
