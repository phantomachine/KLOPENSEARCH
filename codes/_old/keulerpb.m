function fval = keulerpb(k,A,B,C,ETA,SIGMA,KAPPA,ALPHA,BETA,GAMA,...
    ...
    % for the Proportional bargaining in DM model
    ...
    q = (( (SIGMA*KAPPA - PBWEDGE)*C*(ZBAR*K)^(PHI-1))...
    ...
    fval = - DELTA ...
            + (1-1/BETA)/(1-TAU_K) + REL_Pyh*ZBAR*ALPHA*k^(ALPHA-1) ...
                - ( SIGMA*(1+TAU_X)/((1-TAU_K)*U_X(X,B,GAMA))) ...
                    *( KAPPA*gamma_prop(q,K,ZBAR,THETA_B,PHI,ETA,b,C) ...
              + (1-KAPPA)*gamma_prop(q_czech,K,ZBAR,THETA_B,PHI,ETA,b,C) );