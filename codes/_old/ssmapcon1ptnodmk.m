function [lconval, nonlconval] = ssmapcon1ptnodmk(x,ALPHA,SIGMA,DELTA,BETA,GAMA,THETA,KAPPA,...
                                            ZBAR,EPSILON,C,ETA,...
                                            OMEGA_I, OMEGA_F, ...
                                            TAU_X, TAU_H, TAU_K,...
                                             ~,~,~,PHI)
...
%      f4 = velocity - GDP / phi_M;
   ...
   %   f5 = ALPHA + log(LS*ZBAR)/log(K/H);
    ...
    nonlconval = [f1,f3,f6];%[f1,f2,f3,f6];      % Nonlinear binding constraints